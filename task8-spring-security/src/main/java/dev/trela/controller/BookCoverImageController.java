...
public class BookCoverImageController {
    ...
    public BookCoverImageController(BookService bookService, GridFSBucket gridFSBucket, MessageService messageService) {
    ...
    public ResponseEntity<String> uploadImage(
        ...
        return ResponseEntity.ok(messageService.getMessage("book.cover.upload.success"));
    }
    ...
    public ResponseEntity<StreamingResponseBody> downloadImage(
        ...
        if(book.getImageFileId() == null){
            throw new ResourceNotFoundException(messageService.getMessage("book.cover.not.found"));
        }

        GridFSFile file = gridFSBucket.find(eq("_id", new ObjectId(book.getImageFileId()))).first();

        if (file == null){
            throw new ResourceNotFoundException(messageService.getMessage("book.cover.file.not.found"));
        }
